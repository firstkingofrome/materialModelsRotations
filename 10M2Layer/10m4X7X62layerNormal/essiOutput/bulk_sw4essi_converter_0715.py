...
#essiLine,drm_filename,sw4HDF5OutExt = sw4Inputs[0],drm_filename,".cycle=00000.essi" 
def child(essiLine,drm_filename,sw4HDF5OutExt=".cycle=00000.essi"):
    ...
    # input metadata
    drm_filename = sw4OptionsDict["file"].split('.')[0] + sw4OptionsDict["file"].split('.')[1] + ".essiHDF5"
...
if __name__ == '__main__':
    ...
    sw4InputFile = "10m4X7X62layerNormal.sw4input"
    ...
    processList = []    
    #child(sw4Inputs[0],drm_filename,sw4HDF5OutExt=".cycle=00000.essi")
    
    #prepare all of the processess
    for i in sw4Inputs:
        processList.append(Process(target=child, args=(i,drm_filename)))
        #copy the essi container so that each process has its own
        subprocess.Popen("cp " + drm_filename + " "+parseOutESSIParameters(i)["file"].split('.')[0] + parseOutESSIParameters(i)["file"].split('.')[1]+".essiHDF5", shell=True)
    #release
