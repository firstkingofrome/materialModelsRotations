...
class velocityField():
    ...
    def integrateAll(self,taper=20):
        #you might consider prefiltering this if you have any problems
        #compute fouier transforms of all of these so that I can use spectral derrivative
        self.x = np.fft.rfft(self.x[:,:,:,:],axis=0)
        self.y = np.fft.rfft(self.y[:,:,:,:],axis=0)
        self.z = np.fft.rfft(self.z[:,:,:,:],axis=0)
        #multiply by 1/jw
        self.x= self.x/1j
        self.y=self.y/1j
        self.z=self.z/1j
        #go back
        self.x=np.fft.irfft(self.x[:,:,:,:],axis=0)
        self.y=np.fft.irfft(self.y[:,:,:,:],axis=0)
        self.z=np.fft.irfft(self.z[:,:,:,:],axis=0)  
        #recompute time series
        #self.t = self.t[:-1]
    
    #diff. all records in the volume
    #taper indicates the number of time steps that I need to taper at the beginning and the end
    def differAll(self):
        #you might consider filtering this
        #compute fouier transforms of all of these so that I can use spectral derrivative
        self.x = np.fft.rfft(self.x[:,:,:,:],axis=0)
        self.y = np.fft.rfft(self.y[:,:,:,:],axis=0)
        self.z = np.fft.rfft(self.z[:,:,:,:],axis=0)
